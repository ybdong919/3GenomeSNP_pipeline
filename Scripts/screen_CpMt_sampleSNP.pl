...
foreach (@input){
    ...
    if (/(\d+(\.)*\d*)\t\.\t\DP/) {
        #print "$1\n";
        $QT1{$_} = $1 unless $1 < 0;         ### Q-value control
    }    
}
...
my $input2 = "SNP_step1_".$filename;
my $output2 = "SNP_step2_".$filename;
...
my $input3 = "SNP_step2_".$filename;
my $output3 = "SNP_step3_".$filename;
...
my $input4 = "SNP_step3_".$filename;
my $output4 = "SNP_step4_".$filename;
...
open(FILENAME2,'>>', 'SNP_names_of_step4') or die;
